...
def fetch_month_data(supabase, table_name, date_obj, date_column='date', page_size=1000):
    ...
    Args:
        ...
        page_size: Number of rows to fetch per request (default: 1000)
...
def fetch_filtered_table(supabase, table_1, table_2, col_1, col_2, col_1_value=None):
    # Step 1: Fetch all rows from table_1 where col_1 == max(col_1) or col_1 == col_1_value
    if col_1_value is None:
        # Get the max value of col_1
        max_response = (
            supabase
            .table(table_1)
            .select(col_1)
    ...
    # Fetch all rows from table_1 where col_1 equals the filter value
    table_1_rows = []
    start = 0
    page_size = 1000
    while True:
    ...
    # Step 2: Get unique values from col_2 in table_1_rows
    ...
    # Step 3: Fetch all rows from table_2 where col_2 is in col_2_values
    table_2_rows = []
    start = 0
    while True:
        response = (
    ...
    return pl.DataFrame(table_1_rows), pl.DataFrame(table_2_rows)